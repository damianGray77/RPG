...
WindowWin32::WindowWin32(
	...
	WindowWin32::query_perf_freq = 0;
	class_atom = NULL;
	instance   = NULL;

	this->    draw_callback =     draw_callback;
	this->  resize_callback =   resize_callback;
	this->sizemove_callback = sizemove_callback;

	self = this;
}

bool WindowWin32::init(const uint32 width, const uint32 height) {
	timeBeginPeriod(1);

	LARGE_INTEGER li;
	if (QueryPerformanceFrequency(&li)) {
		WindowWin32::query_perf_freq = 1000000.0 / double(li.QuadPart);
	}
...
void WindowWin32::unload() {
	...
	UnregisterClassW(class_atom, instance);
}

// This is a static method. This is my workaround to get it to play nicely within an instanced object.
LRESULT CALLBACK WindowWin32::proc(HWND window, uint msg, WPARAM wparam, LPARAM lparam) {
	return self->_proc(window, msg, wparam, lparam);
}

LRESULT CALLBACK WindowWin32::_proc(HWND window, uint msg, WPARAM wparam, LPARAM lparam) {
	LRESULT ret = 0;
...
bool WindowWin32::init_window(const uint32 width, const uint32 height) {
	...
	wc.lpszClassName = get_unique_classname();
...
void WindowWin32::update_client_size() {
	RECT client_rect;
...
uint8 WindowWin32::map_key(const WPARAM wparam) {
	switch (wparam) {
